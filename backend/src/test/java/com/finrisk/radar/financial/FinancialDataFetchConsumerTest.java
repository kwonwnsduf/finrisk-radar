...
class FinancialDataFetchConsumerTest {
  @Test
  void publishesFetchedEventAfterCollectionCompletes() {
    ...
    FinancialDataFetchConsumer consumer =
        new FinancialDataFetchConsumer(collections, logs, publisher);
    UUID jobId = UUID.randomUUID();
    when(collections.collect(jobId))
        .thenReturn(
            new FinancialCollectionResult(
                jobId, 1L, "005930", "00126380", 2024, 4, "CFS", false, 1, "s3://bucket/raw.json"));

    consumer.consume(
        new FinancialDataFetchRequestedEvent(jobId, 1L, "005930", 2024, 4, null, Instant.now()));
