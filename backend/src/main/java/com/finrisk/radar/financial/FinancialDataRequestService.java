...
public class FinancialDataRequestService {
  ...
  public FinancialDataRequestService(
  ...
  public FinancialMetricFetchResponse request(Long userId, FinancialMetricFetchRequest request) {
    return request(userId, request, null);
  }

  public FinancialMetricFetchResponse requestForRisk(
      Long userId, FinancialMetricFetchRequest request, UUID riskCalculationJobId) {
    if (riskCalculationJobId == null) throw new BusinessException(ErrorCode.INVALID_INPUT);
    return request(userId, request, riskCalculationJobId);
  }

  private FinancialMetricFetchResponse request(
      Long userId, FinancialMetricFetchRequest request, UUID riskCalculationJobId) {
    ...
    FinancialCollectionLog log =
        logs.createRequested(
            userId,
            asset.getId(),
            stockCode,
            request.year(),
            request.quarter(),
            riskCalculationJobId);
    try {
      publisher.publishRequestedAndAwait(
          new FinancialDataFetchRequestedEvent(
              log.getJobId(),
              asset.getId(),
              stockCode,
              request.year(),
  ...
  private Asset resolveAsset(FinancialMetricFetchRequest request) {
    ...
    return assets
        .findFirstByTickerOrderByIdAsc(request.stockCode().trim())
        .orElseThrow(() -> new BusinessException(ErrorCode.ASSET_NOT_FOUND));
  }

  private String stockCode(FinancialMetricFetchRequest request, Asset asset) {
    return request.stockCode() == null || request.stockCode().isBlank()
        ? asset.getTicker()
