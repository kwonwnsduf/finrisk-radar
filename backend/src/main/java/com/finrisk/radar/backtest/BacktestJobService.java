...
public class BacktestJobService {
	...
	public BacktestJobService(BacktestJobRepository jobs, BacktestResultRepository results, ObjectMapper objectMapper) {
		this.jobs = jobs;
	...
	public void completeWithResult(UUID jobId, BacktestCalculationResult calculation) {
		...
		results.save(BacktestResult.from(jobId, calculation,
				toJson(calculation.monthlyReturns()), toJson(calculation.dailyPortfolioValues()), toJson(calculation.trades())));
		job.complete();
	}
