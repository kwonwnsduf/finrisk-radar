...
import { DocumentRiskSection } from "@/components/documents/document-risk-section";
...
export function AssetDetail({ assetId }: { assetId: number }) {
  ...
  const addMutation = useMutation({
    ...
    onError: (error) =>
      setNotice(apiErrorMessage(error, "관심자산을 추가하지 못했습니다.")),
  });
      ...
      <Card className="border-slate-200">
        ...
        <CardContent>
          ...
          {notice ? (
            <p
              role="status"
              className="mt-4 rounded-xl bg-slate-100 px-4 py-3 text-sm text-slate-700"
            >
              {notice}
            </p>
          ) : null}
